...
const Agency = {
  ...
  update(id, payload) {
    ...
    return AgencyModel.findOneAndUpdate({_id: id, is_archived: false}, payload, {
      new: true,
      runValidators: true
    });
  },
  ...
  setContracts(id, payload) {
    const {rahat, rahat_admin, token, updated_by} = payload;
    return AgencyModel.findOneAndUpdate(
      {_id: id, is_archived: false},
      {
    ...
    try {
      assets.forEach(async el => {
        ...
        await AgencyModel.findOneAndUpdate(
          {_id: id, 'kobotool_assets.asset_id': {$nin: assetIds}},
          {
